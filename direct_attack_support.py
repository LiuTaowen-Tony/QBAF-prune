...
def main(dataset_name, model, X, y, *, model_name, visualise, nth_run, 
    ...
    prune_model(model, X_train, y_train, X_test, y_test, 
                visualise=is_visualise, nth_run=nth_run, dataset_name=dataset_name,
                model_name=model_name, is_fuzzy=is_fuzzy, lr=lr, decay=decay)
    # torch.save(model.state_dict(), f'{dataset_name}_{model_name}_{is_fuzzy}_{nth_run}.pt')
    file_name = f'{dataset_name}_{model_name}_{is_fuzzy}.csv'
    import os
    if file_name not in os.listdir():
        with open(file_name, 'w') as f:
            writer = csv.writer(f)
            writer.writerow(['connections', 'test_acc', 'precision', 'recall', 'f1'])
    with open(f'{dataset_name}_{model_name}_{is_fuzzy}.csv', 'a') as f:
        test_acc, precision, recall, f1 = test(model, X_test, y_test)
        writer = csv.writer(f)
        writer.writerow(
            [
                str(model.get_connections()),
                round(test_acc, 4),
                round(precision, 4),
                round(recall, 4),
                round(f1, 4),
    ...
    for dataset_name in ['iris', 'adult', 'mushroom']:
        ...
        for i in range(10):
            main(dataset_name, model, X, y, 
                    ...
                    nth_run = i, is_fuzzy = is_fuzzy, 
                    lr = 0.01, decay = 0.0001)
