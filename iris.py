...
def main():
    ...
    model = iris_model(X.shape[1], 10, y.shape[1])


    
    for i in range(100):
        model = train(model, X_train, y_train, X_test, y_test, 1000, 0.001, 0)
        test(model, X_test, y_test)
        conn1 = model.linear1.weight.nonzero()
        conn2 = model.linear2.weight.nonzero()
        print(f'conn1: {conn1}')
        print(f'conn2: {conn2}')
        time.sleep(1)

        visualize_neural_network([conn1, conn2])
        ...
        if not flag1 and not flag2:
            cv2.waitKey(0)
            torch.save(model.state_dict(), 'adult_model.pt')
            test(model, X_test, y_test)
            return
